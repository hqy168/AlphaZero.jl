...
const ALIGNMENTS =
  ...
  XY = [
    ...
    [[(i, i) for i in 1:N]];
    [[(i, M - i + 1) for i in 1:N]]]
  for k in 1:M - N
    for m in 1:M - N
      XY2 = [
        [[(i, j) for j in k + 1:k + N] for i in m + 1:m + N];
        [[(i, j) for i in k + 1:k + N] for j in m + 1:m + N];
        [[(i, i) for i in k + 1:k + N]];
        [[(i, M - i + 1) for i in k + 1:k + N]]]  
...
function GI.parse_action(::GameSpec, str)
  ...
  n = pos_of_xy((x, y))
  (0 < n < NUM_POSITIONS + 1) ? n : nothingg
end
