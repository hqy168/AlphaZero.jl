...
function place_stones!(board, color, stones)
  for s in stones
    (col, row) = s
    # board[s...] = color
    setindex!(board, color, col, row)
  end
end

function find_reached(board, c, env::GameEnv)
  color = board[c...]
  chain = Set([c])
  reached = Set()
  frontier = [c]
...
function is_koish(board, c, env::GameEnv)
  # Check if c is surrounded on all sides by 1 color, and return that color
  if board[c...] != EMPTY return nothing end
  neighs = Set(board[n...] for n in env.neighbors[c...])
...
function is_eyeish(board, c, env)
  # Check if c is an eye, for the purpose of restricting MC rollouts.
  color = is_koish(board, c, env)
  if color == nothing
    return nothing
  end

  diag_faults = 0
  diag = env.diagonals[c...]
  if length(diag) < 4
    diag_faults += 1
  end
...
function from_board(board, env::GameEnv)
  ...
  for color ∈ (WHITE, BLACK)
    while color ∈ board
      ...
      coord = coordify(coord_flat, N = env.boardSize)
  ...
  liberty_counts = zeros(UInt8, env.boardSize, env.boardSize)
...
function _handle_captures!(lib_trac::LibertyTracker, captured_stones, env::GameEnv)
...
function add_stone!(lib_trac::LibertyTracker, color, c, env::GameEnv)
  ...
  _handle_captures!(lib_trac, captured_stones, env)
...
mutable struct GoPosition
  ...
  function GoPosition(env::GameEnv; board = nothing, n = 0, komi = 7.5,
    caps = (0, 0), lib_tracker = nothing, ko = nothing,
    recent = Vector{PlayerMove}(), board_deltas = nothing, to_play = BLACK)

    b = board !== nothing ? board : copyboard_from(env)
...
function copyboard_from(env::GameEnv) 
  b = zeros(Int8, env.boardSize, env.boardSize)
  for col in 1:env.boardSize
    for row in 1:env.boardSize
      b[col, row] = env.board[col, row]
    end
  end
  return b
end
...
function deepcopy(pos::GoPosition)
  ...
  return GoPosition(pos.env; board = new_board, n = pos.n, komi = pos.komi, caps = pos.caps,
...
function is_move_suicidal(pos::GoPosition, move)
  potential_libs = Set()
  for n in pos.env.neighbors[move...]
...
function all_legal_moves(pos::GoPosition)
...
function flip_playerturn!(pos::GoPosition; mutate = false)
  new_pos = mutate ? pos : deepcopy(pos)
  new_pos.ko = nothing
  new_pos.to_play *= -1
  return new_pos
...
function play_move!(pos::GoPosition, c; color = nothing, mutate = false)
  ...
  potential_ko = is_koish(new_pos.board, c, pos.env)

  place_stones!(new_pos.board, color, [c])
  captured_stones = add_stone!(new_pos.lib_tracker, color, c, pos.env)
...
function result_string(pos::GoPosition)
  points = score(pos)
  if points > 0
...
function replay_position(pos::GoPosition, result)
  ...
  =#
  pos.n == length(pos.recent) ? nothing : throw(AssertionError("GoPosition history is incomplete"))
  replay_buffer = Vector{PositionWithContext}()
  dummy_pos = GoPosition(pos.env; komi = pos.komi)
